...
p <- ggplot(dati,aes(MR.Delay,nWBV,color=Group))
...
p=ggplot(dati, aes(x = MR.Delay, y = nWBV, colour = Group, group = Subject.ID)) + geom_line() + geom_point()
p

mod=lmer(nWBV ~ MR.Delay*Group+(1+MR.Delay|Subject.ID),data=dati)
summary(mod)
anova(mod)
...
library(ggplot2)
p=ggplot(dati, aes(x = MR.Delay, y = nWBV0, colour = Group, group = Subject.ID)) + geom_line() + geom_point()
p
...
dati1lev=dati %>%
  group_by(Subject.ID) %>%
  summarize(estim=(coefficients(lm(nWBV ~ MR.Delay))))
...
Il modello testato: `MR.Delay*Group+(1|Subject.ID)` ammette solo intercetta casuale, mentre il coefficiente angolare (cioè l'evoluzione nel tempo) è uguale per tutti i soggetti (dello stesso Group vista l'interazione imposta nella formula). E' una assunzione ragionevole?

Potremmo quindi voler testare un modello più flessibile, con coefficiente angolare specifico per ogni soggetto: `MR.Delay*Group+(1+MR.Delay|Subject.ID)`
